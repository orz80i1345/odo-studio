...
import type { Booking, BookingPaymentProof, CreateBookingInput, EquipmentItem, ID, Paginated } from '../../types'
import {
  filter,
  toBooking,
  toBookingCreate,
...
export async function createBooking(api: ApiClient, input: CreateBookingInput) {
  ...
  const equipmentItemIds = input.equipmentItemIds ?? []
  const timeSlotIds = slots.map((slot) => slot.id)
  if (sceneIds.length === 0) throw new Error('請選擇至少一個佈景。')
  await assertSceneSlotsAvailable(api, sceneIds, timeSlotIds)
  const equipmentItems = await getSelectedEquipmentItems(api, equipmentItemIds)
  await assertEquipmentSlotsAvailable(api, equipmentItemIds, timeSlotIds)
  const totals = await calculateBookingTotals(api, input, slots, equipmentItems)
...
async function calculateBookingTotals(api: ApiClient, input: CreateBookingInput, slots: ReturnType<typeof toTimeSlot>[], equipmentItems: EquipmentItem[] = []) {
  ...
  const equipmentTotal = equipmentItems.reduce((sum, item) => sum + item.unitPrice, 0)
  const subtotal = hourlyPrice * hours + equipmentTotal
...
async function getSelectedEquipmentItems(api: ApiClient, equipmentItemIds: ID[]) {
  if (equipmentItemIds.length === 0) return []
  const page = await listEquipmentItems(api, { activeOnly: true, pageSize: 200 })
  const selected = page.items.filter((item) => equipmentItemIds.includes(item.id))
  if (selected.length !== equipmentItemIds.length) {
    throw new Error('部分器材已停用或不存在，請重新選擇器材。')
  }
  return selected
}

async function assertEquipmentSlotsAvailable(api: ApiClient, equipmentItemIds: ID[], timeSlotIds: ID[]) {
  if (equipmentItemIds.length === 0 || timeSlotIds.length === 0) return
  const existing = await listBookingEquipmentTimeSlots(api, { equipmentItemIds, timeSlotIds })
  if (existing.length > 0) {
    throw new Error('所選器材在此時段已被租借，請重新選擇器材或時段。')
  }
}
